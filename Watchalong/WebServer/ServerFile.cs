...
namespace WebServer
{
    public class ServerFile : PlayableFile
    {
        ...
        public ServerFile(string videoUrl, string audioUrl, string title, double duration, bool isFileAvailable, FileType type) : base (videoUrl, audioUrl, title, duration, isFileAvailable, type)
        {
            UUID = Uuid.GetUuid();
        }
        ...
        /// <param name="isFileAvailable">Whether the file is available</param>
        /// <param name="type">Whether it's stored offline or it's downloaded</param>
        public ServerFile(int uuid, string videoUrl, string audioUrl, string title, double duration, bool isFileAvailable, FileType type) : base(videoUrl, audioUrl, title, duration, isFileAvailable, type)
        {
            UUID = uuid;
        }
    }
